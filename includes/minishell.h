...
t_token				*get_previous_token(t_token *first_token,
						t_token *curr_token);
t_token				*init_token(t_token *token, int len_input);
t_token				*add_token_back(t_lexer *lexer, int len_input);
void				handle_def_1char(char *input, t_token_aux *aux,
						t_lexer *lexer, int *f);
void				handle_quote_1char(char *input, t_token_aux *aux,
						t_lexer *lexer, int *f);
void				handle_def_2char(char *input, t_token_aux *aux,
						t_lexer *lexer, int *f);
void				handle_terminal(char *input, t_token_aux *aux,
						t_lexer *lexer, int *f);
void				handle_start_quote(char *input, t_token_aux *aux,
						t_lexer *lexer, int *f);
void				process_char_def(char *input, t_token_aux *aux,
						t_lexer *lexer);
void				process_char_quote(char *input, t_token_aux *aux,
						t_lexer *lexer);
void				process_char_dquote(char *input, t_token_aux *aux,
						t_lexer *lexer);
int					check_only_terminal(char *input);
void				init_lexer_aux(char *input, t_token_aux *aux,
						t_lexer *lexer);
void				process_char(char *input, t_token_aux *aux, t_lexer *lexer);
void				clean_last_tokens(t_token_aux *aux, t_lexer *lexer);
void				insert_expansion(t_token *token, int sta, int len,
						char *mid_str);
...
char				*ft_strjoin_3(const char *s1, char connector,
						const char *s2);
...
int					builtin_functions(t_ast *node, char **comms, t_px *px,
						int to_exit);
...
void				update_env(char *env_to_change, char *new_env,
						char *to_free);
